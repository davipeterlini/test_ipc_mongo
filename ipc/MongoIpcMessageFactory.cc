...
IpcMessage* MongoIpcMessageFactory::fromMessageType(mongo::BSONObj* mongoMessage) {
	...
	case EXAMPLE_MESSAGE:
		ipcMessage = new ExampleMessage();

		((ExampleMessage*)ipcMessage)->set_a(string_to<bool>(messageContent["a"].String()));
		((ExampleMessage*)ipcMessage)->set_b(string_to<uint32_t>(messageContent["b"].String()));
		((ExampleMessage*)ipcMessage)->set_c(messageContent["c"].String());
...
mongo::BSONObjBuilder* MongoIpcMessageFactory::fromMessageType(IpcMessage* message) {
	...
	case EXAMPLE_MESSAGE:
		content->append("a", to_string<bool>(((ExampleMessage*) message)->get_a()));
		content->append("b", to_string<uint32_t>(((ExampleMessage*) message)->get_b()));
		content->append("c", ((ExampleMessage*) message)->get_c());
		break;
